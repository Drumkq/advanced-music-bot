...
export class DiscordStrategy extends PassportStrategy(Strategy) {
    ...
    async validate(accessToken: string, refreshToken: string, profile: Profile): Promise<IUser> {
        const foundUser = await this.userService.findByDiscordId(profile.id);
        if (!foundUser) {
            return await this.userService.create({discordId: profile.id, favoriteSongs: []}, accessToken);
        }
